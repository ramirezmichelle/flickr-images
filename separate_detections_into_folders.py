...
def process_detection(d,options):
    ...
    # source_path = os.path.join(options.base_input_folder,"~content~flickr_images~" + relative_filename)
    source_path = options.base_input_folder + "~content~flickr_images~" + relative_filename
...
def separate_detections_into_folders(options):
    ...
    for d in detections:
        # fn = d['file']
        d['file'] = d['file'].split("/")[-1] #get image's name (relative)
        fn = d['file']
